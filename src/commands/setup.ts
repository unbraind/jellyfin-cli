...
function configureSetupCommandOptions(cmd: Command): Command {
  return cmd
...
async function runSetup(thisCommand: Command, options: SetupCommandOptions): Promise<void> {
  const existingConfig = getConfig(options.name);
  const runtimeFormat = resolveOutputFormat(thisCommand, options);
  ...
  if (isInteractive && !options.outputFormat) {
    const candidate = (await prompt(`Default output format [${outputFormatChoices()}] (${persistedOutputFormat}): `))
      .trim();
  ...
  if (!apiKey && ((username && !password) || (!username && password))) {
    console.error(formatOutput({ error: 'Username and password must be provided together.' }, runtimeFormat, 'error'));
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(formatOutput({ error: `Could not connect to server at ${serverUrl}` }, runtimeFormat, 'error'));
    process.exit(1);
  }
  ...
  if (!apiKey && username && password) {
    ...
    try {
      ...
      console.error(formatOutput({ error: 'Authentication failed. Check username and password.' }, runtimeFormat, 'error'));
      process.exit(1);
    }
  } else if (!apiKey && !username) {
    console.error(
      formatOutput({ error: 'Either --api-key or --username/--password is required' }, runtimeFormat, 'error'),
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (!userId && users.length > 0) {
      ...
      const matchedUser = username ? users.find((u) => u.Name?.toLowerCase() === usernameLower) : undefined;
...
export function createSetupCommand(): Command {
  ...
  configureSetupCommandOptions(cmd.description('Interactive setup wizard for configuring the CLI'))
    .action(async function (this: Command, options: SetupCommandOptions) {
      await runSetup(this, options);
    });

  configureSetupCommandOptions(
    cmd.command('wizard').description('Run setup wizard explicitly (alias of base `setup`)'),
  ).action(async function (this: Command, options: SetupCommandOptions) {
    await runSetup(this, options);
  });
