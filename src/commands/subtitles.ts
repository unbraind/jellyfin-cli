...
/**
 * Builds the subtitles command tree with validated options and actions.
  ...
  cmd.command('search <itemId> <language>').description('Search for remote subtitles')
    ...
    .action(async (itemId, language, options) => {
      ...
      try {
        ...
        console.log(formatter.formatToon(subtitles.map((s) => ({
          id: s.Id,
          name: s.Name,
          format: s.Format,
          author: s.Author,
  ...
  cmd.command('download <itemId> <subtitleId>').description('Download a remote subtitle')
    ...
    .action(async (itemId, subtitleId, options) => {
      ...
      try {
        ...
        console.log(formatter.formatMessage(`Subtitle ${subtitleId} downloaded to item ${itemId}`, true));
      } catch (err) { handleError(err, format); }
    });
  ...
  cmd.command('delete <itemId> <index>').description('Delete a subtitle track')
    ...
    .action(async (itemId, index, options) => {
      ...
      try {
        ...
        console.log(formatter.formatMessage(`Subtitle track ${index} deleted from item ${itemId}`, true));
      } catch (err) { handleError(err, format); }
    });
  ...
  cmd.command('providers').description('List subtitle providers')
    ...
    .action(async (options) => {
      ...
      try {
        ...
        console.log(formatter.formatToon(providers.map((p) => ({ name: p.Name })), 'subtitle_providers'));
      } catch (err) { handleError(err, format); }
    });
